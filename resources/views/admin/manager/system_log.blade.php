...
<div class="page-container">
	<form action="/admin/manager/log" method="get">
		<div class="text-c"> 日期范围：
			...
			-
			<input type="text" name="stop_time" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:120px;">
			<input type="text" name="username" id="" placeholder="搜用户名" style="width:250px" class="input-text">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<span class="r">共有数据：<strong>{{$num or 'error'}}</strong> 条</span>
	</div>
	<table class="table table-border table-bordered table-bg table-hover table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$row -> id}}</td>
				<td>@if($row -> status == 1)登录成功 @elseif($row -> status == 2) 登录失败 @endif</td>
				<td>{{$row -> username}}</td>
				<td>{{$row -> address}}</td>
				<td>{{$row -> area}}</td>
				<td>{{$row -> location}}</td>
...
$(function(){
	...
	$('.table-sort').dataTable({
		"aLengthMenu": [[10, 25, 50, -1], ["10", "25", "50", "All"]],//二组数组，第一组数量
